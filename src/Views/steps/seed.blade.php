...
@section('page_title', __($namespace . '::seed.title'))

@section('content')
    <p class="pb-3 text-gray-800">@lang($namespace . '::seed.introduce')</p>
    <form method="POST" action="{{ $url_retry }}">
        @csrf
        @if($result['seed']['value'])
            <div class="flex">
                <pre class="border-2 p-3 m-5 w-full text-xs">{{ $result['seed']['value'] }}</pre>
            </div>
        @endif
